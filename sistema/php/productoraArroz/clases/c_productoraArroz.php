...
class ProductoraArroz
{
    function Crear($datos)
    {
        ...
        $carpeta = "../../imagenes/users/productoraArroz/";
                ...
                //     $respuesta['mensaje'] = "La cedula ya existe";
                    ...
                    if (move_uploaded_file($_FILES['foto']['tmp_name'], $carpeta . $nombreFinal)) {
                        ...
                        if ($tb_productora_arroz) {
                            ...
                            // $tb_usuarios = '1';
                            $tb_usuarios = Conexion::UpdateRegistro("INSERT INTO tb_usuarios (cod_usuario,cod_rol,usuario,correo,password,foto,estado,conexion,usuario_creado,fecha_creado) VALUES  ('" . htmlentities($cod_usuario) . "',3,'" . htmlentities($usuario) . "','" . htmlentities($correo) . "', '" . htmlentities($pass) . "','" . htmlentities($nombreFinal) . "','A','D','" . htmlentities($iduser) . "',now())");
                            if ($tb_usuarios) {
    ...
    {;
        $tb_productora_arroz = Conexion::UpdateRegistro("update tb_productora_arroz set estado = '$estado',  
                                                               fecha_editado = now(), usuario_editado = '$iduser'
                                                               where cod_productoraArroz = '$cod_usuario'  ");
        if ($tb_productora_arroz) {
            $respuesta['status'] = "correcto";
        ...
        if ($_FILES['imagen']['name'] !== '') {
            ...
            if (
                ...
                if (move_uploaded_file($_FILES['imagen']['tmp_name'], $carpeta . $nombreFinal)) {
                    ...
                    if (file_exists('../../imagenes/users/productoraArroz/' . $imagenAntigua)) {
                        unlink('../../imagenes/users/productoraArroz/' . $imagenAntigua);
                    }
    ...
    {
        $iduser            = $_POST['iduser'];
        $cod_productoraArroz = $_POST['cod_productoraArroz'];
        $nombres           = $_POST['nombres'];
        $apellidos         = $_POST['apellidos'];
        $celular           = $_POST['celular'];

        $tb_productora_arroz  = Conexion::UpdateRegistro("UPDATE tb_productora_arroz set 
            nombres = '$nombres', apellidos = '$apellidos', celular = '$celular',
            fecha_editado = now(), usuario_editado = '$iduser'
            where cod_productoraArroz = '$cod_productoraArroz' ");
        if ($tb_productora_arroz) {
            $respuesta['status'] = "correcto";
    ...
    function EliminarCuenta($datos)
    {
        ...
        $cod_usuario    = $_POST['cod_usuario'];
        $cod_rol        = $_POST['cod_rol'];
        $tb_productora_arroz = Conexion::UpdateRegistro("UPDATE tb_usuarios set estado = 'D', usuario_eliminado = '$iduser', fecha_eliminado = now() 
         where cod_usuario = '$cod_usuario' and cod_rol = '$cod_rol' ");
        if ($tb_productora_arroz) {
            $respuesta['status'] = "correcto";
    ...
    function ActualizarEstado($datos)
    {
        ...
        $cod_rol        = $_POST['cod_rol'];
        $estado         = $_POST['estado'];
        $tb_productora_arroz = Conexion::UpdateRegistro("UPDATE tb_usuarios set estado = '$estado', usuario_editado = '$iduser', fecha_editado = now() 
         where cod_usuario = '$cod_usuario' and cod_rol = '$cod_rol' ");
        if ($tb_productora_arroz) {
            $respuesta['status'] = "correcto";
            $respuesta['mensaje'] = "Estado actualizado";
